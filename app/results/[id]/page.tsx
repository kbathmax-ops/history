          ...
          <div className="min-w-0">
            <div className="flex flex-wrap items-center gap-2 mb-2">
              <span
                ...
                style={{ fontFamily: 'var(--font-mono)', color: 'var(--t3)', background: 'var(--bd2)' }}
              >
                {ep.episode_id}
              </span>
...
export default function ResultsPage() {
  ...
  useEffect(() => {
    ...
    // Fast path: result was just computed and is already in sessionStorage
    const stored = sessionStorage.getItem(`sanctions_result_${id}`)
    if (stored) {
      try { setResult(JSON.parse(stored)); return } catch {}
    }

    // Fallback: fetch from API (handles refresh, shared links, bookmarks)
      ...
      .then(json => {
        ...
        setResult({
          ...
          data: {
            ...
            query_filters: json.query_filters ?? {},
          },
        })
      })
  ...
  if (notFound) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] gap-6 px-8">
        ...
        <button
          onClick={() => router.push('/')}
          className="rounded-full border px-4 py-2 text-[10px] uppercase tracking-[0.2em] transition-all hover:opacity-70"
          style={{ fontFamily: 'var(--font-mono)', color: 'var(--t2)', borderColor: 'var(--bd2)' }}
        >
          ← New query
        </button>
      </div>
